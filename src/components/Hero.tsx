...
const portalPreview = [
  { icon: BarChart3, label: "Management", status: "Live", tone: "bg-cyan-50 text-cyan-700", motion: "animate-rise-delay-1" },
  { icon: BookOpen, label: "Teacher", status: "Live", tone: "bg-emerald-50 text-emerald-700", motion: "animate-rise-delay-2" },
  { icon: Users, label: "Parent", status: "Live", tone: "bg-amber-50 text-amber-700", motion: "animate-rise-delay-3" },
  { icon: GraduationCap, label: "Student", status: "Live", tone: "bg-slate-100 text-slate-700", motion: "animate-rise-delay-3" },
];
        ...
        <div className="space-y-7">
          ...
          <div className="space-y-5">
            ...
            <p className="animate-rise-delay-2 max-w-2xl text-lg leading-8 text-slate-600 sm:text-xl">
              EduTrack brings administrators, teachers, parents, and students into one coordinated
              workflow so every attendance mark, grade update, and message moves instantly.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:max-w-2xl lg:grid-cols-4">
            {portalPreview.map((item) => (
              ...
              >
                <div className={`mb-2 inline-flex rounded-lg px-2.5 py-1 text-xs font-semibold ${item.tone}`}>
                  <item.icon className="mr-1 h-3.5 w-3.5" />
                  {item.label}
                </div>
                <p className="text-xs font-mono uppercase tracking-wide text-emerald-700">{item.status}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative">
          <div className="glass-panel animate-rise rounded-3xl p-6 shadow-xl shadow-slate-900/10 sm:p-8">
            <div className="mb-6 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900">Unified School Pulse</h3>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-cyan-200 bg-cyan-50 p-4">
                <p className="text-sm font-semibold text-cyan-900">Attendance updates</p>
                <p className="mt-1 text-sm text-cyan-800">Teacher marks attendance once, Parent + Admin dashboards update immediately.</p>
              </div>
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                <p className="text-sm font-semibold text-emerald-900">Grade visibility</p>
                <p className="mt-1 text-sm text-emerald-800">Published exams flow from teacher gradebook to student and parent portals in real time.</p>
              </div>
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                <p className="text-sm font-semibold text-amber-900">Secure by default</p>
                <p className="mt-1 text-sm text-amber-800">Role-based access keeps every record visible only to the right audience.</p>
              </div>
            </div>

            <div className="mt-6 flex items-center gap-2 text-sm font-semibold text-slate-700">
              <ShieldCheck className="h-4 w-4 text-teal-700" />
              Powered by Next.js + Supabase RLS
            </div>
          </div>
